...
shape_empresas <- rgdal::readOGR(dsn="Tutoriais/tema_escolas",layer="tema_escolas")
...
calc_sde(id=1, filename="Tutoriais/empresas_Output.txt", centre.xy=NULL, calccentre=TRUE,
...
plot_sde(plotnew=FALSE, plotSDEaxes=FALSE, plotweightedpts=FALSE,
         titletxt="Elipse de Desvio Padr?o das Escolas",
         plotpoints=FALSE, plotcentre=TRUE, sde.col="red")

# exporta a elipse para um shapefile
# ?ltimo par?metro é: 1-point, 2-polyline, 5-polygon
shape_elipse <- convert.to.shapefile(sdeloc, sdeatt, "id", 5)
write.shapefile(shape_elipse,"Tutoriais/elipse_escolas/elipse_escolas",arcgis=T)
